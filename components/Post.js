  ...
  return (
    <div className="bg-white my-7 border rounded-sm">
        ...
        {comments.length > 0 && (
            <div className="ml-10 h-20 overflow-scroll scrollbar-thumb-black scrollbar-thin">
                {comments.map(comment => (
                    ...
                    <p
                    className="text-sm flex-1"><span className="font-bold">{comment.data().username}</span>{" "}{comment.data().comment}</p>
        ...
        {session && (
            ...
            <input onChange={e => setComment(e.target.value)} value={comment} placeholder="Adicionar Comentário..." className="border-none flex-1 focus:right-0 outline-none" type="text" />
            <button type="submit" disabled={!comment.trim()} onClick={sendComment} className="font-semibold text-blue-400 cursor-pointer">Postar</button>
            </form>
